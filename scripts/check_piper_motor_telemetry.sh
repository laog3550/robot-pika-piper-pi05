...
usage() {
  cat <<'EOF'
Usage: scripts/check_piper_motor_telemetry.sh [--config PATH]
                                                [--duration SECONDS] <left|right>

Passively summarize Piper high-speed motor feedback. This command never
transmits CAN frames and does not print motor positions or CAN payloads.
EOF
}
...
PYTHONPATH="$repo_root/src/pi05_control/src${PYTHONPATH:+:$PYTHONPATH}" \
  exec /usr/bin/python3 "$script_dir/check_piper_motor_telemetry.py" \
    --interface "$PI05_CAN_INTERFACE" --label "$side" --duration "$duration"
