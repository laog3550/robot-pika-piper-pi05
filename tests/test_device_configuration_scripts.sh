...
if [[ "$*" == '-brief link show type can' ]]; then
  ...
  [[ "${MOCK_CAN_MODE:-}" == ambiguous ]] && printf 'can1 DOWN <NOARP,ECHO>\n'
...
#!/usr/bin/env bash
printf '/dev/ttyUSB0\n'
[[ "${MOCK_SERIAL_MODE:-}" == ambiguous ]] && printf '/dev/ttyUSB1\n'
EOF
...
PI05_CAN_INTERFACE=can0
PI05_CAN_BITRATE=1000000
PI05_CAN_USB_BUS_INFO=9-9
PI05_PIKA_SERIAL_ALIAS=/dev/pi05-pika-right
PI05_PIKA_SERIAL_BAUD=460800
PI05_PIKA_SERIAL_ID_PATH=pci-0000:ff:ff.f-usb-0:9.9:1.0
PI05_PIKA_SERIAL_VENDOR_ID=dead
PI05_PIKA_SERIAL_MODEL_ID=beef
PI05_ARM_SIDE=right
...
PATH="$fake_bin:$PATH" "$repo_root/scripts/configure_can.sh" check --config "$config" >/dev/null
if PATH="$fake_bin:$PATH" "$repo_root/scripts/configure_can.sh" check --config "$config" --yes >/dev/null 2>&1; then
  printf 'CAN check unexpectedly accepted --yes\n' >&2
  exit 1
fi
if PATH="$fake_bin:$PATH" "$repo_root/scripts/configure_can.sh" apply --config "$config" </dev/null >/dev/null 2>&1; then
...
if MOCK_CAN_MODE=ambiguous PATH="$fake_bin:$PATH" \
  "$repo_root/scripts/configure_can.sh" apply --config "$config" --yes >/dev/null 2>&1; then
  printf 'ambiguous CAN identity unexpectedly reached apply\n' >&2
  exit 1
fi
[[ ! -e "$sudo_marker" ]] || { printf 'ambiguous CAN identity invoked sudo\n' >&2; exit 1; }

if PATH="$fake_bin:$PATH" "$repo_root/scripts/configure_pika_serial.sh" check --config "$config" >/dev/null 2>&1; then
  printf 'serial check unexpectedly passed without its alias\n' >&2
  exit 1
fi
if PATH="$fake_bin:$PATH" "$repo_root/scripts/configure_pika_serial.sh" apply --config "$config" </dev/null >/dev/null 2>&1; then
...
if MOCK_SERIAL_MODE=ambiguous PATH="$fake_bin:$PATH" \
  "$repo_root/scripts/configure_pika_serial.sh" apply --config "$config" --yes >/dev/null 2>&1; then
...
PI05_CAN_INTERFACE=\$(touch $test_root/executed)
EOF
if PATH="$fake_bin:$PATH" "$repo_root/scripts/configure_can.sh" check --config "$unsafe_config" >/dev/null 2>&1; then
