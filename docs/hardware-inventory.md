...
状态：**基线草案已冻结；现场值已采集，剩余阻塞项为 H-05、H-08、H-09、H-10。**
...
| H-03L/R | Piper 固件 | 两侧均与固定版本 SDK/ROS 驱动兼容 | 两侧均已采集为 `S-V1.8-2`，并按厂商资料清单选择 `piper_sdk + piper_ros/noetic` 路线 | 分侧记录固件版本，省略设备标识 |
...
| H-06L/R | Pika 手部采集器 | 左右各一套；串口夹爪/IMU；460800 8N1 | 两侧物理映射和稳定别名已验证；位姿与夹爪编码器只读数据已在 S08 和 2026-09-16 联调中验证 | 分侧脱敏 udev 属性和只读日志 |
| H-07L/R | Pika 定位设备 | 左右定位链路可唯一识别且互不串侧 | 两侧均完成真机定位与活动检查，左右未串侧 | 分侧节点、话题类型/频率和遮挡测试 |
...
- 任一侧 H-05/H-09 或公共 H-08/H-10 未确认前，不进入任何使能或运动阶段。
  （历史注：本条在 H-03 未确认时写过，H-03 固件已在 2026-09-16 采集完成；自此以后
  左右单臂已多次真机使能与运动，本条的实际约束对象是 H-05/H-08/H-09/H-10。）
