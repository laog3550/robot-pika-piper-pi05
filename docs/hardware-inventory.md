...
| H-02L/R | Piper 机械臂 | 左右各一台、6 轴、带夹爪 | 右侧设备映射已验证；两侧型号待确认 | 分侧记录型号/用途，不记录序列号 |
| H-03L/R | Piper 固件 | 两侧均与固定版本 SDK/ROS 驱动兼容 | 现场待确认 | 分侧记录固件版本，省略设备标识 |
| H-04L/R | USB-CAN 适配器 | 两只、`gs_usb`、1 Mbps，物理端口与逻辑侧别唯一 | 右侧已验证；左侧待验证 | 分侧记录脱敏驱动和 CAN 状态 |
| H-05L/R | CAN 线束与终端 | 两条独立总线的 CAN-H/CAN-L/GND 与终端正确 | 现场待确认 | 分侧断电测量和复核签字 |
| H-06L/R | Pika 手部采集器 | 左右各一套；串口夹爪/IMU；460800 8N1 | 右侧别名已验证；左侧待验证 | 分侧脱敏 udev 属性和只读日志 |
...
| H-09L/R | Piper 供电 | 两侧电源、保护和接地均符合对应实物手册 | 现场待确认，阻塞项 | 分侧额定值与实测值；省略序列号 |
