...
项目的 `data_msgs`、`piper_msgs`、`piper_description`、`pika_remote_piper` 和 `piper` 是 VCS 清单固定仓库中的源码依赖，不是 rosdep/apt
键；脚本只对这些键使用 `--skip-keys`，避免 rosdep 把源码依赖误报为未知系统包。这不
会安装或豁免它们，运行依赖这些包的节点前仍必须从固定 commit 构建对应 overlay。
